...
async function main() {
    console.log('Starting global menu update for all hotels...');
    ...
    console.log(`Found ${hotels.length} hotels in the system.`);

    // 2. Clear AND Update each hotel
    for (const hotel of hotels) {
        console.log(`Processing hotel: ${hotel.name} (${hotel.id})`);

        // Clear items and categories for THIS hotel
        ...
        console.log(`  Completed menu replacement for ${hotel.name}`);
    }

    console.log('Global menu update completed successfully!');
}
